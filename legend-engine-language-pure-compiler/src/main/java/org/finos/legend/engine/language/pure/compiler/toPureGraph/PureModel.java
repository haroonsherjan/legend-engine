...
public class PureModel implements IPureModel
{
    ...
    public PureModel(PureModelContextData pureModelContextData, CompilerExtensions extensions, Iterable<? extends CommonProfile> pm, ClassLoader classLoader, DeploymentMode deploymentMode, PureModelProcessParameter pureModelProcessParameter, Metadata metaData)
    {
        ...
        try (Scope scope = GlobalTracer.get().buildSpan("Build Pure Model").startActive(true))
        {
            ...
            this.loadFinalPass(pureModelContextDataIndex);
    ...
    public void loadFinalPass(PureModelContextDataIndex pure)
    {
        this.extensions.sortExtraProcessors(pure.stores.keysView()).forEach(p ->
        {
            MutableList<org.finos.legend.engine.protocol.pure.v1.model.packageableElement.store.Store> stores = pure.stores.get(p);
            stores.forEach(this::processFinalPass);
        });
        pure.mappings.forEach(this::processFinalPass);
    }
    ...
    private void processFinalPass(org.finos.legend.engine.protocol.pure.v1.model.packageableElement.PackageableElement element)
    {
        visitWithErrorHandling(element, new PackageableElementFinalPassBuilder(getContext(element)));
    }
